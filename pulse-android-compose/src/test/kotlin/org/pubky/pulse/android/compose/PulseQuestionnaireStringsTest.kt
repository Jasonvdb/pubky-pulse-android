...
/**
 * Pure-JVM parity tests for [PulseQuestionnaireStrings] (the Compose analog of the
 * Swift `PulseQuestionnaireStrings`). No Compose harness / Robolectric — these only
 ...
 *  - [PulseQuestionnaireStrings.with] overrides *only* the passed-in fields and
 ...
 */
class PulseQuestionnaireStringsTest {

    @Test
    fun `DEFAULT carries the Swift catalog defaultValue strings`() {
        val d = PulseQuestionnaireStrings.DEFAULT
    ...
    fun `DEFAULT companion equals a freshly-constructed instance`() {
        assertEquals(PulseQuestionnaireStrings(), PulseQuestionnaireStrings.DEFAULT)
    }

    @Test
    fun `with overrides only the passed field and preserves the rest`() {
        val base = PulseQuestionnaireStrings.DEFAULT
    ...
    fun `with chains so the last override wins per field`() {
        val result = PulseQuestionnaireStrings.DEFAULT
            .with(title = "First")
    ...
    fun `with covers the Android-only builder fields beyond Swift's surface`() {
        ...
        val overridden = PulseQuestionnaireStrings.DEFAULT.with(

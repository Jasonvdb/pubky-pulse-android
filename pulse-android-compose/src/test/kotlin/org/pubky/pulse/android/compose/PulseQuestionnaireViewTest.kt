...
import org.pubky.pulse.android.PulseQuestionnaireChoiceOption
import org.pubky.pulse.android.PulseQuestionnaireQuestion
import org.pubky.pulse.android.PulseQuestionnaireSchema
...
/**
 * Compose UI behavior of [PulseQuestionnaireView], mirroring the navigation +
 * gating surface of the Swift `PulseQuestionnaireFlowContainer`. Renders the view
 * directly (no live `Pulse.configure`), so it exercises the client-side flow that
 ...
 *  - overridden [PulseQuestionnaireStrings] render.
 */
@RunWith(RobolectricTestRunner::class)
@GraphicsMode(GraphicsMode.Mode.NATIVE)
@Config(sdk = [34])
class PulseQuestionnaireViewTest {

    @get:Rule
    val composeRule = createComposeRule()

    private fun spec(): PulseQuestionnaire = PulseQuestionnaire(
        id = "q1",
        slug = "demo",
        name = "Demo",
        ...
        schema = PulseQuestionnaireSchema(
            ...
            questions = listOf(
                PulseQuestionnaireQuestion.SingleChoice(
                    ...
                    required = true,
                    options = listOf(
                        PulseQuestionnaireChoiceOption("r", "Red"),
                        PulseQuestionnaireChoiceOption("b", "Blue"),
                    ),
                ),
                PulseQuestionnaireQuestion.Text(
                    id = "why",
                    title = "Why?",
                    subtitle = null,
                    required = false,
    ...
    fun `consent prompt shows and accepting advances to first question`() {
        composeRule.setContent {
            PulseQuestionnaireView(questionnaire = spec(), showsConsent = true)
        }
    ...
    fun `next is disabled until the required question is answered`() {
        composeRule.setContent {
            PulseQuestionnaireView(questionnaire = spec(), showsConsent = false)
        }
    ...
    fun `overridden strings render`() {
        val custom = PulseQuestionnaireStrings.DEFAULT.with(consentAccept = "Let's go!")
        composeRule.setContent {
            PulseQuestionnaireView(questionnaire = spec(), showsConsent = true, strings = custom)
        }

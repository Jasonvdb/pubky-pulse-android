...
/**
 * Compose UI behavior of [PulseFeedbackView], mirroring the validation surface of
 * the Swift `PulseFeedbackView`. Driven by the Compose test harness under
 * Robolectric (no live `Pulse.configure`), so it exercises the client-side
 * validation gate that runs *before* any `Pulse.sendFeedback` network call:
 ...
 *  - [PulseFeedbackStrings] overrides render (custom header text).
 *
...
class PulseFeedbackViewTest {
    ...
    fun `overridden strings render`() {
        val custom = PulseFeedbackStrings.DEFAULT.with(header = "How can we help you today?")
        composeRule.setContent {
            PulseFeedbackView(showsContactFields = false, strings = custom)
        }
    ...
    fun `only-name contact shows the incomplete-contact error`() {
        val strings = PulseFeedbackStrings.DEFAULT
        composeRule.setContent {
            PulseFeedbackView(showsContactFields = true, strings = strings)
        }
    ...
    fun `cancel action is hidden when no onCancel is provided`() {
        val strings = PulseFeedbackStrings.DEFAULT
        composeRule.setContent {
            PulseFeedbackView(showsContactFields = false, strings = strings, onCancel = null)
        }

...
/**
 * Pure-JVM parity tests for [PulseFeedbackStrings] (the Compose analog of the
 * Swift `PulseFeedbackStrings`) and [PulseFeedbackActionsPlacement]. No Compose
 ...
 *  - [PulseFeedbackStrings.with] overrides *only* the passed-in field and leaves
 *    every sibling untouched — the analog of Swift's `with(header:…)` copy.
 *  - a no-arg `with()` is a value-equal copy.
 *  - [PulseFeedbackActionsPlacement] enumerates exactly the Swift cases.
 */
class PulseFeedbackStringsTest {

    @Test
    fun `DEFAULT carries the Swift catalog defaultValue strings`() {
        val d = PulseFeedbackStrings.DEFAULT
    ...
    fun `DEFAULT companion equals a freshly-constructed instance`() {
        ...
        // (mirrors Swift's `static let default = PulseFeedbackStrings()`).
        assertEquals(PulseFeedbackStrings(), PulseFeedbackStrings.DEFAULT)
    }

    @Test
    fun `with overrides only the passed field and preserves the rest`() {
        val base = PulseFeedbackStrings.DEFAULT
    ...
    fun `with chains so the last override wins per field`() {
        val result = PulseFeedbackStrings.DEFAULT
            .with(header = "First")
    ...
    @Test
    fun `PulseFeedbackActionsPlacement enumerates exactly the Swift cases`() {
        // Swift: case toolbar, case inline. Default in PulseFeedbackView is TOOLBAR.
        assertEquals(
            listOf(PulseFeedbackActionsPlacement.TOOLBAR, PulseFeedbackActionsPlacement.INLINE),
            PulseFeedbackActionsPlacement.entries.toList(),
        )
        assertSame(
            PulseFeedbackActionsPlacement.TOOLBAR,
            PulseFeedbackActionsPlacement.valueOf("TOOLBAR"),
        )
        assertSame(
            PulseFeedbackActionsPlacement.INLINE,
            PulseFeedbackActionsPlacement.valueOf("INLINE"),
        )
    }

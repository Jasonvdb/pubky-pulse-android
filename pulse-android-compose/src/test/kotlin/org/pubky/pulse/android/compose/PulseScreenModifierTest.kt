...
/**
 * Behavior of the [pulseScreen] Compose modifier, mirroring the Swift
 * `.pulseScreen(_:)` view modifier's `onAppear` / `onDisappear` lifecycle:
 ...
 *
 * The appear/disappear side effects in production call `Pulse.debug(...)`. To
 ...
 * contract `pulseScreen` relies on — then a smoke test confirms the real
 * `Modifier.pulseScreen(...)` composes and renders its content.
 */
@RunWith(RobolectricTestRunner::class)
@GraphicsMode(GraphicsMode.Mode.NATIVE)
@Config(sdk = [34])
class PulseScreenModifierTest {

    @get:Rule
    val composeRule = createComposeRule()

    @Test
    fun `real pulseScreen modifier composes and renders content`() {
        composeRule.setContent {
            Box(modifier = Modifier.pulseScreen("Home")) {
                Text("home-content")
            }
        }
    ...
    @Test
    fun `real pulseScreen modifier survives a full enter then exit lifecycle`() {
        // Drive the real production modifier through mount → unmount so its
        // actual onAppear (Pulse.debug "sdk:screen_appeared") and onDispose
        // (Pulse.debug "sdk:screen_disappeared" with _duration_ms) closures run.
        // Pulse is unconfigured here — Pulse.debug must no-op safely (matching the
        ...
        composeRule.setContent {
            if (present) {
                Box(modifier = Modifier.pulseScreen("Detail")) {
                    Text("detail-content")
                }
            } else {
                Text("after-content")
    ...
    /**
     * Structural twin of the production [pulseScreen] modifier: a `DisposableEffect`
     * keyed on [name] that fires [onAppear] on enter and [onDispose] on exit.
     * Lets the test count appear/dispose firings without a configured `Pulse`
     * transport — the firing pattern is exactly what `pulseScreen` relies on.
     */

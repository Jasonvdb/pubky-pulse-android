...
import org.pubky.pulse.android.PulseFeedbackReceipt
import kotlinx.coroutines.launch

/**
 * Where the Submit (and Cancel) actions live. The Compose analog of Swift's
 * `PulseFeedbackActionsPlacement`.
 */
public enum class PulseFeedbackActionsPlacement {
    /**
 ...
 * and email) and submits it to Pubky Pulse via [Pulse.sendFeedback]. The Compose
 * analog of the Swift `PulseFeedbackView`.
 *
 ...
 * actions placement is [PulseFeedbackActionsPlacement.TOOLBAR], which renders a
 * Cancel/Submit row at the top; use [PulseFeedbackActionsPlacement.INLINE] when
...
public fun PulseFeedbackView(
    ...
    var submitted by remember { mutableStateOf<PulseFeedbackReceipt?>(null) }
    ...
    // submitting flag, calls Pulse.sendFeedback, maps PulseFeedbackError cases onto
    ...
    Column(modifier = modifier.fillMaxWidth()) {
        if (actionsPlacement == PulseFeedbackActionsPlacement.TOOLBAR && submitted == null) {
            FeedbackActionsRow(
                strings = strings,
        ...
        if (actionsPlacement == PulseFeedbackActionsPlacement.INLINE && submitted == null) {
            InlineActionsBar(
                strings = strings,
                isSubmitting = isSubmitting,
                canSubmit = canSubmit,
...
/** Top action row (Cancel + Submit) for [PulseFeedbackActionsPlacement.TOOLBAR]. */
@Composable
private fun FeedbackActionsRow(
    strings: PulseFeedbackStrings,
...
/** Bottom action bar (Submit + optional Cancel) for [PulseFeedbackActionsPlacement.INLINE]. */
@Composable

...
import org.pubky.pulse.android.PulseQuestionnaireAnswerStore
import org.pubky.pulse.android.PulseQuestionnaireDraft
import org.pubky.pulse.android.PulseQuestionnaireError
import org.pubky.pulse.android.PulseQuestionnaireQuestion
import org.pubky.pulse.android.PulseQuestionnaireReceipt
import kotlinx.coroutines.launch

/**
 * A Compose view that renders a [PulseQuestionnaire] as a step-through flow (one
 * question per page, with a progress bar) and submits via
 * [Pulse.saveQuestionnaireResponse]. The Compose analog of the Swift
 ...
 */
@Composable
public fun PulseQuestionnaireView(
...
/** Phase of the questionnaire flow. Mirrors Swift's `PulseQuestionnairePhase`. */
private sealed interface QuestionnairePhase {
    ...
    data class Success(val receipt: PulseQuestionnaireReceipt) : QuestionnairePhase
}

/**
 * Internal host that owns the questionnaire flow: phase machine, answer state,
 * submit/dismiss. The Compose analog of Swift's `PulseQuestionnaireFlowContainer`
 ...
 */
@Composable
internal fun PulseQuestionnaireFlowContainer(
    questionnaire: PulseQuestionnaire,
    modifier: Modifier = Modifier,
    inProgress: PulseQuestionnaireDraft? = null,
    showsConsent: Boolean = false,
    ...
    var answers by remember(questionnaire.id, inProgress) {
        mutableStateOf(
            if (inProgress != null) {
                PulseQuestionnaireAnswerStore().prefilled(inProgress.answers)
            } else {
    ...
    fun emitConsentShown() {
        Pulse.info("sdk:questionnaire_consent_shown", attributes = mapOf("_slug" to questionnaire.slug))
    }

    fun acceptConsent() {
        Pulse.info("sdk:questionnaire_started", attributes = mapOf("_slug" to questionnaire.slug))
    ...
    fun finishSuccess(receipt: PulseQuestionnaireReceipt) {
    ...
    suspend fun saveDraftInBackground() {
        ...
        runCatching {
            Pulse.saveQuestionnaireResponse(slug = questionnaire.slug, answers = payload, isComplete = false)
        }
    ...
    suspend fun submit() {
        ...
        try {
            ...
            Pulse.info("sdk:questionnaire_submitted", attributes = mapOf("_slug" to questionnaire.slug))
...
private fun RunningPage(
    ...
    answers: PulseQuestionnaireAnswerStore,
    strings: PulseQuestionnaireStrings,
    isSubmitting: Boolean,
    onAnswersChange: (PulseQuestionnaireAnswerStore) -> Unit,
    onCancel: () -> Unit,
    onBack: () -> Unit,
    onNext: () -> Unit,
    onSubmit: () -> Unit,
    ...
    Column(modifier = Modifier.fillMaxWidth()) {
        ...
        PulseQuestionnaireProgressBar(current = index, total = total)

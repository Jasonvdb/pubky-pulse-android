...
import org.pubky.pulse.android.PulseQuestionnaireState
import org.pubky.pulse.android.PulseQuestionnaireTrigger
import kotlinx.coroutines.launch

/**
 * Auto-present a Pubky Pulse questionnaire when [trigger]'s conditions hold and
 * the user is eligible per server-side state. The Compose analog of Swift's
 * `.pulseQuestionnaire(slug:trigger:...)` view modifier.
 *
 ...
 * overlays a [ModalBottomSheet] hosting [PulseQuestionnaireView] once the trigger
 ...
 * ([Pulse.questionnaireWasShownThisProcess]) plus the in-memory [presented] flag
...
@Composable
public fun PulseQuestionnaireGate(
    slug: String,
    ...
    suspend fun evaluate() {
        ...
        val passesLocalGate = runCatching {
            ...
            if (Pulse.questionnaireWasShownThisProcess(slug)) return@runCatching false
            if (trigger.isManual) return@runCatching false
            val snapshot = PulseQuestionnaireState.shared?.snapshot() ?: return@runCatching false
        ...
        val result = runCatching { Pulse.fetchQuestionnaire(slug = slug, force = forceShow) }.getOrNull() ?: return
        val questionnaire = result.questionnaire ?: return
        if (!forceShow) {
            Pulse.markQuestionnaireShown(slug)
        }
        presented = true
...
private data class Presentation(
    val questionnaire: PulseQuestionnaire,
    val inProgress: PulseQuestionnaireDraft?,
)

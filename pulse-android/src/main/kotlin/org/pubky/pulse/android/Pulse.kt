...
/**
 * Public entry point to the Pubky Pulse SDK. It can be [configure]d — building an
 * [PulseConfiguration], collecting [DeviceInfo], generating a session id, deriving
 ...
 * the server-side identity *claim* — mirroring the Swift `Pulse` singleton, which
...
public object Pulse {
    ...
    // In-flight `Pulse.log(...)` coroutine accounting — the analog of Swift's
    ...
    internal data class State(
        ...
        // PulseQuestionnaireState.shared so the Compose gate can read it.
        val questionnaireState: PulseQuestionnaireState,
     ...
     * id. `null` before [configure]. Mirrors Swift `Pulse.currentUserId`.
     */
    ...
    public fun configure(
        ...
        val configuration = PulseConfiguration.create(
            context = appContext,
            endpoint = endpoint,
            apiKey = apiKey,
        ...
        val crashSafetyHandler = CoroutineExceptionHandler { _, throwable ->
            android.util.Log.e(
                ConsoleLogger.TAG,
                "Pubky Pulse background task failed; event(s) may be dropped (host app unaffected).",
                throwable,
            )
        }
        ...
        val duplicateFilter = DuplicateFilter()
        val questionnaireState = PulseQuestionnaireState(appContext)
        val attachmentUploader = AttachmentUploader(
            endpoint = configuration.endpoint.toURL(),
            apiKey = configuration.apiKey,
            scope = scope,
        ...
        synchronized(lock) {
            ...
            // Context. Mirrors Swift's `PulseQuestionnaireState.shared`.
            PulseQuestionnaireState.shared = questionnaireState
        }
        ...
        // Mirrors Swift's `PulseQuestionnaireState.shared.markConfiguredOnce()` at
     ...
     * Swift `Pulse.setUser(_:)`, which persists unconditionally.
     *
     ...
     * `Pulse.log(...)` coroutines ([awaitInFlightLogTasks]) so the claim's
     ...
     * may be shared between users). Mirrors Swift `Pulse.clearUser(newAnonymousId:)`,
     ...
     * with the current screen. Mirrors Swift `Pulse.info(_:)`.
     *
    ...
    public fun info(
        ...
        log(message, PulseLogLevel.INFO, screenName, cleanAttributes(attributes), attachments, file, function, line)
    }

    /** Log a **debug**-level event. See [info] for parameter semantics. Mirrors Swift `Pulse.debug(_:)`. */
    public fun debug(
        message: String,
        screenName: String? = null,
        attributes: Map<String, String?> = emptyMap(),
        ...
        log(message, PulseLogLevel.DEBUG, screenName, cleanAttributes(attributes), attachments, file, function, line)
    }

    /** Log a **warn**-level event. See [info] for parameter semantics. Mirrors Swift `Pulse.warn(_:)`. */
    public fun warn(
        message: String,
        screenName: String? = null,
        attributes: Map<String, String?> = emptyMap(),
        ...
        log(message, PulseLogLevel.WARN, screenName, cleanAttributes(attributes), attachments, file, function, line)
    }

    /** Log an **error**-level event from a free-text message. See [info] for parameter semantics. Mirrors Swift `Pulse.error(_:)`. */
    public fun error(
        message: String,
        screenName: String? = null,
        attributes: Map<String, String?> = emptyMap(),
        ...
        log(message, PulseLogLevel.ERROR, screenName, cleanAttributes(attributes), attachments, file, function, line)
    }
     ...
     * separate issues. Mirrors Swift `Pulse.error(_:Error)`.
     *
     * Pass an optional [message] to override the auto-derived event message with
     * caller context (e.g. `Pulse.error(e, "while loading photos")`). SDK-owned
    ...
    public fun error(
        ...
        log(extracted.message, PulseLogLevel.ERROR, screenName, merged, attachments, file, function, line)
    }
     ...
     * string value to remove a property. Mirrors Swift `Pulse.setUserProperties`.
     *
     ...
     * with [setUser] — so any in-flight `Pulse.log(...)` coroutines reach the
     ...
     * per process). Backs `PulseQuestionnaireCondition.Launches`. Mirrors Swift
     * `Pulse.launchCount`. Zero before the first [configure].
     */
    public val launchCount: Int
        get() = PulseQuestionnaireState.shared?.launchCount ?: 0

    /**
     * Total number of foreground transitions since install. Backs
     * `PulseQuestionnaireCondition.Foregrounds`. Mirrors Swift `Pulse.foregroundCount`.
     */
    public val foregroundCount: Int
        get() = PulseQuestionnaireState.shared?.foregroundCount ?: 0

    /**
     * Epoch-millis timestamp of the first-ever [configure] on this install, or
     * null if none recorded. Mirrors Swift `Pulse.firstLaunchAt` (a `Date` there;
     * epoch millis here, matching [PulseQuestionnaireState]).
     */
    public val firstLaunchAt: Long?
        get() = PulseQuestionnaireState.shared?.firstLaunchAt
     ...
     * `Pulse.fetchQuestionnaire(slug:force:)`.
     *
     ...
     *
     * @throws PulseQuestionnaireError.NotConfigured if [configure] has not run.
     * @throws PulseQuestionnaireError.SlugNotFound if the slug doesn't exist.
     * @throws PulseQuestionnaireError.ServerError / TransportFailure on failure.
     */
    public suspend fun fetchQuestionnaire(
        slug: String,
        force: Boolean = false,
    ): PulseQuestionnaireFetchResult {
        val snapshot = transportSnapshot() ?: throw PulseQuestionnaireError.NotConfigured
     ...
     * Swift `Pulse.saveQuestionnaireResponse`.
     */
     ...
     * Swift `Pulse.dismissQuestionnaires`. Returns the server's `dismissed_at`.
     *
     * @throws PulseQuestionnaireError.NotConfigured if [configure] has not run.
     */
    public suspend fun dismissQuestionnaires(): java.util.Date {
        ...
        if (snapshot == null || userId == null) throw PulseQuestionnaireError.NotConfigured
     ...
     * Mirrors Swift `Pulse._debugClearShownQuestionnaires`.
     */
    ...
    private fun transportSnapshot(): QuestionnaireTransportSnapshot? = synchronized(lock) {
        ...
        if (s == null) {
            if (!hasWarnedNotConfigured) {
                ...
                android.util.Log.w(
                    ConsoleLogger.TAG,
                    "Pulse.configure() has not been called. Questionnaire call dropped.",
                )
            }
    ...
    /**
     * Submit free-text user feedback to Pubky Pulse. Mirrors Swift
     * `Pulse.sendFeedback`.
     *
     ...
     * [PulseFeedbackReceipt]) and surfaces success/failure in its own UI. The
     ...
     * throws [PulseFeedbackError.EmptyMessage] before any network call. [name] and
     ...
     *
     * @throws PulseFeedbackError.NotConfigured if [configure] has not been called.
     * @throws PulseFeedbackError.EmptyMessage if [message] is blank.
     * @throws PulseFeedbackError.ServerError on a non-2xx response.
     * @throws PulseFeedbackError.TransportFailure on a network/encode/decode failure.
     */
    public suspend fun sendFeedback(
        ...
        if (trimmed.isEmpty()) throw PulseFeedbackError.EmptyMessage
        ...
        val snapshot: FeedbackSnapshot? = synchronized(lock) {
            ...
            if (s == null) {
                if (!hasWarnedNotConfigured) {
                    ...
                    android.util.Log.w(
                        ConsoleLogger.TAG,
                        "Pulse.configure() has not been called. sendFeedback dropped.",
                    )
                }
                return@synchronized null
            }
        ...
        if (snapshot == null) throw PulseFeedbackError.NotConfigured
        ...
        return when (val result = transport.submitFeedback(body)) {
            is FeedbackResult.Success -> {
                ...
                // "Pulse.kt:sendFeedback" (Swift uses #file/#function/#line of the
     ...
     * `"step:<stepName>"`. Mirrors Swift `Pulse.step(_:)`.
     *
     ...
     * `Pulse.track(_:)` which also forwards to `step`.
     */
    ...
    /**
     * Start a tracked operation. Returns a [PulseOperation] whose [complete][PulseOperation.complete],
     * [fail][PulseOperation.fail], or [cancel][PulseOperation.cancel] method should be
     * called when the operation finishes. Mirrors Swift `Pulse.startOperation`.
     *
    ...
    public fun startOperation(
        ...
        val op = PulseOperation(metric = slug)
     ...
     * `metric:<slug>:record` event. Mirrors Swift `Pulse.recordMetric`.
     *
     * The [metric] slug should contain only lowercase letters, numbers, and
    ...
    private const val SDK_PACKAGE: String = "org.pubky.pulse.android"
     ...
     * on exit. Mirrors Swift's private `Pulse.log(...)`.
     *
     * Before [configure] this is a no-op (warned once via Logcat), matching
     * Swift's "Pulse.configure() has not been called. Events are being dropped."
     */
    internal fun log(
        message: String,
        level: PulseLogLevel,
        ...
        val snapshot: LogSnapshot = synchronized(lock) {
            ...
            if (s == null) {
                if (!hasWarnedNotConfigured) {
                    ...
                    android.util.Log.w(
                        ConsoleLogger.TAG,
                        "Pulse.configure() has not been called. Events are being dropped.",
                    )
                }
                return
            }
     ...
     * suspending analog of Swift's `Pulse.shutdown()`. Call before a deliberate
    ...
    internal fun resetForTesting() {
        ...
        synchronized(lock) {
            ...
            pendingLogDrains.clear()
            hasWarnedNotConfigured = false
            PulseQuestionnaireState.shared = null
        }
        synchronized(shownLock) { shownSlugs.clear() }

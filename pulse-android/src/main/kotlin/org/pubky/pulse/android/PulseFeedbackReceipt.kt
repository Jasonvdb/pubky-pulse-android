 ...
 * through [Pulse.sendFeedback]. Mirrors the Swift `FeedbackRequestBody`: every
...
public data class PulseFeedbackReceipt(
    ...
    internal companion object {
         ...
         */
        fun fromJson(json: JSONObject): PulseFeedbackReceipt {
            val id = json.optString("id")
            val createdAtRaw = json.optString("created_at", "")
            val createdAt = parseIso8601(createdAtRaw) ?: Date()
            return PulseFeedbackReceipt(id = id, createdAt = createdAt)
        }
...
/**
 * Errors surfaced by [Pulse.sendFeedback]. The public analog of the Swift
 * `PulseFeedbackError` enum; [message] (via the overridden [Throwable.message])
 * mirrors Swift's `errorDescription` so a thrown error reads identically.
 */
public sealed class PulseFeedbackError(message: String) : Exception(message) {
    /** [Pulse.configure] has not been called yet. */
    public object NotConfigured : PulseFeedbackError(
    ...
    public object EmptyMessage : PulseFeedbackError("Feedback message is empty.")

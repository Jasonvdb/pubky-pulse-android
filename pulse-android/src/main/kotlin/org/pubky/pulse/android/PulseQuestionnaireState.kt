 ...
 * analog of the Swift SDK's `PulseQuestionnaireState`.
 *
 ...
 * reload of `Pulse.configure(...)` doesn't double-count the launch.
 *
 ...
 * `Pulse.configure(...)` time and held by [Pulse]. A process-wide [shared] handle is
 * still published from `configure(...)` so the Compose trigger gate and the
 * static [Pulse] accessors can read the counters without threading a [Context]
 * through the call sites.
 */
public class PulseQuestionnaireState internal constructor(
    ...
    /**
     * Called once per process from the tail of [Pulse.configure]. Increments
    ...
    public fun markConfiguredOnce(now: Long = System.currentTimeMillis()) {
        synchronized(lock) {
            ...
            // type-mismatched key, and this runs synchronously in Pulse.configure() on
    ...
    /** Total `Pulse.configure(...)` completions since install. */
    public val launchCount: Int
        // getInt throws ClassCastException on a corrupt/type-mismatched key; this is
        // public API (Pulse.launchCount) a host may read anytime, so degrade to 0
    ...
    /**
     * Epoch-millis timestamp of the first-ever `Pulse.configure(...)` on this
    ...
    public companion object {
         ...
         * Compose trigger gate + the static [Pulse] launch/foreground accessors can
         ...
         */
        @Volatile
        public var shared: PulseQuestionnaireState? = null
            internal set
    }
}

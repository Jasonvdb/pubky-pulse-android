 ...
 *
 * Created by [Pulse.startOperation] — **do not instantiate directly**. The start
 ...
 * `duration_ms`.
 *
 * Mirrors the Swift `PulseOperation` final class. Swift measures elapsed time with
 ...
 */
public class PulseOperation internal constructor(
    /** The metric slug this operation tracks (already normalized by [Pulse.startOperation]). */
     ...
     * event. Mirrors Swift `PulseOperation.complete`.
     *
     ...
     * Mirrors Swift `PulseOperation.fail`.
     */
    public fun fail(
        error: String,
        attributes: Map<String, String?> = emptyMap(),
    ...
    internal companion object {
        // Kept in step with Pulse's source-location placeholders — Kotlin has no
        // #file/#function literal, so an unspecified call site defaults here and
        // Pulse.info/error derives the real frame from the stack.

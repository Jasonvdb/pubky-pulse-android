 ...
 * `PulseQuestionnaire.swift`. Public so consumers can render a spec manually via
 * the Compose `PulseQuestionnaireView`.
 *
 ...
 * omitted. The decode path mirrors Swift's polymorphic `PulseQuestionnaireQuestion`
 * decoder that dispatches on the `type` discriminator.
 */
public data class PulseQuestionnaire(
    public val id: String,
    public val slug: String,
    public val name: String,
    public val description: String?,
    public val schema: PulseQuestionnaireSchema,
) {
    internal companion object {
        /**
         * Decode a questionnaire spec from the server's JSON. Mirrors Swift's
         * `PulseQuestionnaire: Codable`. Throws [PulseQuestionnaireParseException] on
         ...
         */
        fun fromJson(json: JSONObject): PulseQuestionnaire {
            ...
            val schemaJson = json.optJSONObject("schema")
                ?: throw PulseQuestionnaireParseException("questionnaire missing schema")
            return PulseQuestionnaire(
                id = id,
                slug = slug,
                name = name,
                description = description,
                schema = PulseQuestionnaireSchema.fromJson(schemaJson),
            )
        }
    }
}

public data class PulseQuestionnaireSchema(
    public val version: Int,
    public val questions: List<PulseQuestionnaireQuestion>,
) {
    internal companion object {
        fun fromJson(json: JSONObject): PulseQuestionnaireSchema {
            val version = json.optInt("version", 1)
            val arr = json.optJSONArray("questions") ?: JSONArray()
            val questions = ArrayList<PulseQuestionnaireQuestion>(arr.length())
            for (i in 0 until arr.length()) {
                val q = arr.optJSONObject(i) ?: continue
                questions.add(PulseQuestionnaireQuestion.fromJson(q))
            }
            return PulseQuestionnaireSchema(version = version, questions = questions)
        }
    }
}

public data class PulseQuestionnaireChoiceOption(
    public val id: String,
    public val label: String,
) {
    internal companion object {
        fun fromJson(json: JSONObject): PulseQuestionnaireChoiceOption =
            PulseQuestionnaireChoiceOption(
 ...
 * polymorphic `PulseQuestionnaireQuestion` enum — a sealed class so the `when`
 ...
 */
public sealed class PulseQuestionnaireQuestion {
    ...
    public data class SingleChoice(
        ...
        public override val subtitle: String?,
        public override val required: Boolean,
        public val options: List<PulseQuestionnaireChoiceOption>,
    ) : PulseQuestionnaireQuestion()
    ...
    internal companion object {
         ...
         * mirrors Swift's polymorphic `PulseQuestionnaireQuestion.init(from:)`.
         * The wire types are snake_case (`single_choice`, `multi_choice`).
         */
        fun fromJson(json: JSONObject): PulseQuestionnaireQuestion {
            ...
            return when (type) {
                ...
                else -> throw PulseQuestionnaireParseException("unknown question type: $type")
            }
        }

        private fun decodeOptions(json: JSONObject): List<PulseQuestionnaireChoiceOption> {
            val arr = json.optJSONArray("options") ?: JSONArray()
            val out = ArrayList<PulseQuestionnaireChoiceOption>(arr.length())
            for (i in 0 until arr.length()) {
                val o = arr.optJSONObject(i) ?: continue
                out.add(PulseQuestionnaireChoiceOption.fromJson(o))
            }
            return out
        }
    }
 ...
 * `PulseQuestionnaireAnswerValue` enum.
 */
public sealed class PulseQuestionnaireAnswerValue {
    public data class TextValue(public val value: String) : PulseQuestionnaireAnswerValue()
    /** A single option id. */
    public data class ChoiceValue(public val value: String) : PulseQuestionnaireAnswerValue()
    /** A set of option ids. */
    public data class ChoicesValue(public val value: List<String>) : PulseQuestionnaireAnswerValue()
    /** 1..scale. */
    public data class RatingValue(public val value: Int) : PulseQuestionnaireAnswerValue()
    /** 0..10. */
    public data class NpsValue(public val value: Int) : PulseQuestionnaireAnswerValue()
}
 ...
 * The public analog of Swift's `PulseQuestionnaireReceipt`.
 */
public data class PulseQuestionnaireReceipt(
    public val id: String,
 ...
 * public analog of Swift's `PulseQuestionnaireDraft`.
 */
public data class PulseQuestionnaireDraft(
    public val responseId: String,
    public val answers: Map<String, PulseQuestionnaireAnswerValue>,
)

/**
 * Result of [Pulse.fetchQuestionnaire]. When [questionnaire] is non-null the
 ...
 * Swift's `PulseQuestionnaireFetchResult`.
 */
public data class PulseQuestionnaireFetchResult(
    public val questionnaire: PulseQuestionnaire?,
    public val inProgress: PulseQuestionnaireDraft? = null,
    public val ineligibleReason: PulseQuestionnaireIneligibleReason? = null,
)

/**
 * Reason a questionnaire is not eligible to present right now. Mirrors the
 * server's eligibility envelope (and Swift's `PulseQuestionnaireIneligibleReason`).
 * [wire] is the raw server reason string.
 */
public enum class PulseQuestionnaireIneligibleReason(public val wire: String) {
    ...
    internal companion object {
        ...
        fun fromWire(value: String?): PulseQuestionnaireIneligibleReason? {
...
/**
 * Errors surfaced by [Pulse.fetchQuestionnaire] / [Pulse.saveQuestionnaireResponse] /
 * [Pulse.dismissQuestionnaires]. Non-eligible fetches return a result with a null
 * questionnaire instead of throwing. The public analog of Swift's
 * `PulseQuestionnaireError` enum; the [message] mirrors Swift's `errorDescription`
 * so a thrown error reads identically.
 */
public sealed class PulseQuestionnaireError(message: String) : Exception(message) {
    /** [Pulse.configure] has not been called yet. */
    public object NotConfigured : PulseQuestionnaireError(
        "Pubky Pulse is not configured. Call Pulse.configure(...) first.",
    )

    /** The slug doesn't exist on the server (404). */
    public object SlugNotFound : PulseQuestionnaireError("Questionnaire slug not found.")

    /** The server rejected the answers (400). [detail] is the body verbatim. */
    public data class InvalidAnswers(public val detail: String) :
        PulseQuestionnaireError("Invalid answers: $detail")
    ...
    public data class TransportFailure(public val detail: String) :
        PulseQuestionnaireError(detail)
}

/** Thrown internally when a questionnaire/schema/question JSON payload is malformed. */
internal class PulseQuestionnaireParseException(message: String) : Exception(message)

// MARK: - Wire encoding helpers (internal)

/**
 * Encode a `Map<String, PulseQuestionnaireAnswerValue>` into the wire shape
 * (`{ questionId: string | string[] | int }`). Mirrors Swift's
 * `PulseQuestionnaireAnswersWire` encoder.
 */
internal fun encodeAnswers(answers: Map<String, PulseQuestionnaireAnswerValue>): JSONObject {
    val obj = JSONObject()
    for ((key, value) in answers) {
        when (value) {
            is PulseQuestionnaireAnswerValue.TextValue -> obj.put(key, value.value)
            is PulseQuestionnaireAnswerValue.ChoiceValue -> obj.put(key, value.value)
            is PulseQuestionnaireAnswerValue.ChoicesValue -> obj.put(key, JSONArray(value.value))
            is PulseQuestionnaireAnswerValue.RatingValue -> obj.put(key, value.value)
            is PulseQuestionnaireAnswerValue.NpsValue -> obj.put(key, value.value)
        }
    }
    return obj
}
 ...
 * int }`) onto the strongly-typed [PulseQuestionnaireAnswerValue] map by
...
): Map<String, PulseQuestionnaireAnswerValue> {
    ...
    for (question in schema.questions) {
        ...
        when (question) {
            is PulseQuestionnaireQuestion.Text ->
                (value as? String)?.let { out[question.id] = PulseQuestionnaireAnswerValue.TextValue(it) }
            is PulseQuestionnaireQuestion.SingleChoice ->
                (value as? String)?.let { out[question.id] = PulseQuestionnaireAnswerValue.ChoiceValue(it) }
            is PulseQuestionnaireQuestion.MultiChoice ->
                (value as? JSONArray)?.let { arr ->
                    ...
                    out[question.id] = PulseQuestionnaireAnswerValue.ChoicesValue(list)
                }
            is PulseQuestionnaireQuestion.Rating ->
                intFromJson(value)?.let { out[question.id] = PulseQuestionnaireAnswerValue.RatingValue(it) }
            is PulseQuestionnaireQuestion.Nps ->
                intFromJson(value)?.let { out[question.id] = PulseQuestionnaireAnswerValue.NpsValue(it) }
        }
    }
    return out
}
 ...
 * `dismissed_at`) — same shape as [PulseFeedbackReceipt]'s. Tolerates both

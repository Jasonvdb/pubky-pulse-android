...
/**
 * End-to-end attachment behavior through the public [Pulse] API, mirroring the
 * Swift attachment integration: `Pulse.error(..., attachments: [...])` emits the
...
class PulseAttachmentTest {
    ...
    fun `error with attachment reserves and uploads tagged with the event id`() {
        ...
        Pulse.error(
            ...
            attachments = listOf(PulseAttachment.bytes("screenshot-bytes".toByteArray(), name = "shot.png", contentType = "image/png")),
        )
        ...
        Pulse.setUser("real-${System.nanoTime()}")
    ...
    fun `error without attachments triggers no attachment traffic`() {
        configure()
        Pulse.error(message = "plain error")
        Pulse.setUser("real-${System.nanoTime()}")

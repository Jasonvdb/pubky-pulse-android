...
/**
 * End-to-end logging behavior of [Pulse], mirroring the Swift `PulseIntegrationTests`
 * + `SetUserRaceTests`: log levels stamp the right `level`, null attributes are
 * filtered ([Pulse.cleanAttributes]), `error(Throwable)` populates `_error_*`,
 ...
 * A recording [HttpClient] is installed via [Pulse.httpClientOverrideForTesting]
...
class PulseLoggingTest {
    ...
    fun logLevelsStampTheCorrectLevel() {
        configure()
        Pulse.info("an info", screenName = "Home")
        Pulse.debug("a debug")
        Pulse.warn("a warn")
        Pulse.error("an error")
    ...
    fun nullAttributesAreFilteredOut() {
        configure()
        Pulse.info("attr-test", attributes = mapOf("present" to "yes", "absent" to null))
    ...
    fun errorThrowablePopulatesErrorAttributes() {
        ...
        Pulse.error(RuntimeException("save failed", cause), "while saving")
    ...
    fun duplicateEventsAreSuppressedPastTheCap() {
        ...
        repeat(15) { Pulse.info("spammy", screenName = "S") }
    ...
    fun claimPostArrivesAfterAllPriorIngestPosts() {
        ...
        repeat(30) { Pulse.info("burst_$it", screenName = "race") }
